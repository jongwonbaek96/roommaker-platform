...
> 게임 id: `dragon-heart` · 판타지 · 퍼즐 32개 · 등재 2026-07-10
...
| 항목 | 값 |
|---|---|
| 게임 본문 | `games/dragon-heart.html` (단일 파일 자체완결형) |
| 제작 MD | `games/dragon-heart/docs/*.md` — **관리자 전용, 정적 접근 차단됨** |
| registry | `registry.json` → id `dragon-heart` |
...
```json
{
  "id": "dragon-heart",
  "title": "용의 심장",
  "subtitle": "이그나로크의 둥지에서 살아나가라",
  "genre": "판타지",
  "file": "games/dragon-heart.html",
  "status": "완성",
  "puzzles": 32,
  "accent": "#ff6b2b",
  "bg": "#0c0604",
...
- 구매자는 `/play/dragon-heart` 에서 **토큰(RM-XXXX-XXXX)** 입력 → 세션 쿠키(7일) → `/games/dragon-heart.html` 접근 허용.
...
- 이 폴더의 MD에는 **정답이 전부** 들어 있다. `/games/dragon-heart/docs/*` 는 미들웨어와 함수 라우트 **양쪽에서 404**로 막혀 있어 어떤 경로로도 정적 서빙되지 않는다.
