# 색깔없는세상 : 수능만점 — 배포 · 자산 경로

> 게임 id: `colorless-blue` · 잔혹동화 방탈출 · 퍼즐 30개 · 등재 2026-07-08
...
| 항목 | 값 |
|---|---|
| 게임 본문 | `games/colorless-blue.html` + 공용 엔진(`colorless-core*.js`, `colorless-style.css`) |
| 제작 MD | `games/colorless-blue/docs/*.md` — **관리자 전용, 정적 접근 차단됨** |
| registry | `registry.json` → id `colorless-blue` |
...
```json
{
  "id": "colorless-blue",
  "title": "색깔없는세상 : 수능만점",
  "subtitle": "파랑의 그림일기 — 에피소드 3/5",
  "genre": "잔혹동화 방탈출",
  "file": "games/colorless-blue.html",
  "status": "완성",
  "puzzles": 30,
  "accent": "#6d8cff",
  "bg": "#05070f",
...
- 구매자는 `/play/colorless-blue` 에서 **토큰(RM-XXXX-XXXX)** 입력 → 세션 쿠키(7일) → `/games/colorless-blue.html` 접근 허용.
...
- 이 폴더의 MD에는 **정답이 전부** 들어 있다. `/games/colorless-blue/docs/*` 는 미들웨어와 함수 라우트 **양쪽에서 404**로 막혀 있어 어떤 경로로도 정적 서빙되지 않는다.
