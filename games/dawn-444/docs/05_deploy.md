...
> 게임 id: `dawn-444` · 감성 심리극 · 퍼즐 20개 · 등재 2026-07-06
...
| 항목 | 값 |
|---|---|
| 게임 본문 | `games/dawn-444.html` (단일 파일 자체완결형) |
| 제작 MD | `games/dawn-444/docs/*.md` — **관리자 전용, 정적 접근 차단됨** |
| registry | `registry.json` → id `dawn-444` |
...
```json
{
  "id": "dawn-444",
  "title": "새벽 4시 44분",
  "subtitle": "가장 어두운 시간, 해 뜨기 2시간 32분 전",
  "genre": "감성 심리극",
  "file": "games/dawn-444.html",
  "status": "완성",
  "puzzles": 20,
  "accent": "#8fb4ff",
  "bg": "#07070f",
...
- 구매자는 `/play/dawn-444` 에서 **토큰(RM-XXXX-XXXX)** 입력 → 세션 쿠키(7일) → `/games/dawn-444.html` 접근 허용.
...
- 이 폴더의 MD에는 **정답이 전부** 들어 있다. `/games/dawn-444/docs/*` 는 미들웨어와 함수 라우트 **양쪽에서 404**로 막혀 있어 어떤 경로로도 정적 서빙되지 않는다.
